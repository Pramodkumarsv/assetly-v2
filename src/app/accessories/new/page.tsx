...
import { generateAssetTag, ACCESSORY_TYPES, ASSET_STATUSES } from '@/lib/utils'
...
export default function NewAccessoryPage() {
  ...
  const [form, setForm] = useState({
    assetTag: generateAssetTag('ACC'),
    name: '', type: 'KEYBOARD', status: 'AVAILABLE',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    let imageUrl = null
    if (imageFile) imageUrl = await uploadFile(imageFile, 'accessories')
  ...
  // Show success + QR screen after creation
  if (createdItem) {
    return (
      <div className="p-8 max-w-lg">
        <div className="card p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-white mb-1">Accessory Created!</h2>
          ...
          <p className="text-xs text-zinc-500 mb-6">Download the QR code and stick it on the accessory for easy scanning.</p>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Link href="/accessories/new" onClick={() => { setCreatedItem(null); setForm({ assetTag: generateAssetTag('ACC'), name: '', type: 'KEYBOARD', status: 'AVAILABLE', serialNumber: '', brand: '', model: '', purchaseDate: '', purchasePrice: '', warrantyExpiry: '', location: '', department: '', assignedTo: '', notes: '' }) }} className="btn-secondary justify-center text-center">
              Add Another Accessory
            </Link>
            <Link href="/accessories" className="btn-ghost justify-center text-center">View All Accessories</Link>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-8">
        ...
        <h1 className="page-title mt-2">Add New Accessory</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="card p-6 space-y-4">
          ...
          <div>
            ...
            <input className="input" placeholder="e.g. Logitech MX Keys — Desk 4" required value={form.name} onChange={e => set('name', e.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="label">Type *</label>
              <select className="select" value={form.type} onChange={e => set('type', e.target.value)}>
                {ACCESSORY_TYPES.map(t => <option key={t} value={t}>{t.replace('_', ' ')}</option>)}
              </select>
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input className="input" placeholder="Logitech, Apple, Dell..." value={form.brand} onChange={e => set('brand', e.target.value)} />
            </div>
            <div>
              <label className="label">Model</label>
              <input className="input" placeholder="MX Keys, Magic Mouse..." value={form.model} onChange={e => set('model', e.target.value)} />
            </div>
          </div>
        </div>

        <div className="card p-6 space-y-4">
          <h2 className="text-sm font-semibold text-zinc-300">Purchase & Warranty</h2>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" className="input" placeholder="4500" value={form.purchasePrice} onChange={e => set('purchasePrice', e.target.value)} />
            </div>
            <div>
        ...
        <div className="card p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="input" placeholder="Engineering, HR..." value={form.department} onChange={e => set('department', e.target.value)} />
            </div>
          </div>
          <div>
            <label className="label">Location</label>
            <input className="input" placeholder="Office Desk 4, Warehouse..." value={form.location} onChange={e => set('location', e.target.value)} />
          </div>
          <div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Link href="/accessories" className="btn-secondary">Cancel</Link>
        </div>
      </form>
    </div>
  )
...
// Inline QR canvas for the success screen
